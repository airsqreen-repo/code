 ...
 * @version $Id$
 */
public class DoubleStringArrayType implements UserType {
    ...
    public Object nullSafeGet(ResultSet resultSet, String[] strings, SessionImplementor sessionImplementor, Object o)
		...
		if (value == null) {
			...
			String[][] values = new String[tokenizer.countTokens()][2];
			int i = 0;
			while (tokenizer.hasMoreTokens()) {

                String token = tokenizer.nextToken();

                StringTokenizer tokenizerPoint = new StringTokenizer(token, ":");
				values[i][0] = tokenizerPoint.nextToken();
                values[i][1] = tokenizerPoint.nextToken();
			    i = i + 1;
            }
			return values;
		}
    ...
    public void nullSafeSet(PreparedStatement preparedStatement, Object o, int i, SessionImplementor sessionImplementor) throws HibernateException, SQLException {
		...
		if (o == null) {
			...
			for (int j = 0; j < strings.length; j++) {
				buffer.append(  strings[j][0] + ":" + strings[j][1]);
